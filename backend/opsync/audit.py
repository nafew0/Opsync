...
def _normalize_audit_value(value):
    ...
    if isinstance(value, dict):
        normalized = {key: _normalize_audit_value(item) for key, item in value.items()}
        return {
            key: item
    ...
    if isinstance(value, (list, tuple, set)):
        normalized_items = [_normalize_audit_value(item) for item in value]
...
def build_audit_payload(
    ...
    **fields,
    ...
    normalized = {key: _normalize_audit_value(value) for key, value in payload.items()}
    return {
        key: value
