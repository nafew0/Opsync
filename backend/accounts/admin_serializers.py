...
class SiteSettingsAdminSerializer(serializers.ModelSerializer):
    ...
    def get_rate_limit_storage_meta(self, obj):
        ...
        if getattr(settings, "IS_PRODUCTION", False) and (
            not use_redis or not is_shared_backend
        ):
            warning = (
        ...
        return {
            ...
            "trusted_proxy_ips_configured": bool(
...
class SiteSettingsUpdateSerializer(serializers.Serializer):
    ...
    branding_login_banner = serializers.ImageField(
        required=False, allow_empty_file=False
    )
    branding_register_banner = serializers.ImageField(
        required=False, allow_empty_file=False
    )
    ...
    ai_model_openai = serializers.CharField(
        required=False, allow_blank=True, max_length=100
    )
    ai_model_anthropic = serializers.CharField(
        required=False, allow_blank=True, max_length=100
    )
