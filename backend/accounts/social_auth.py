...
def get_callback_state_data(request, provider):
    normalized = normalize_provider(provider)
    state = (request.GET.get("state") or request.POST.get("state") or "").strip()
    if not state:
    ...
    if not state_data:
        raise SocialAuthStateError(
            "The social login session has expired or is invalid."
        )
    ...
    return (
        ...
        state_data,
        frontend_origin,
        sanitize_next_path(state_data.get("next_path")),
    )
...
def update_social_account(
    ...
    *,
    ...
    social_account.last_login_at = (
        timezone.now() if last_login else social_account.last_login_at
    )
    social_account.save()
...
def build_frontend_callback_url(frontend_origin, **params):
    ...
    callback_path = (
        callback_path if callback_path.startswith("/") else f"/{callback_path}"
    )
    query = urlencode(
