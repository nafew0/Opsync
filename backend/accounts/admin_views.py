...
from opsync.audit import log_audit_event
...
def build_user_growth_over_time():
    ...
    rows = (
        User.objects.filter(created_at__gte=first_month)
    ...
    return [
        {
            "label": shift_month(first_month, offset).strftime("%b %Y"),
            "count": counts_by_month.get(
                (
                    shift_month(first_month, offset).year,
                    shift_month(first_month, offset).month,
                ),
                0,
            ),
        }
...
class AdminDashboardView(AdminAPIView):
    def get(self, request):
        ...
        recent_signups = AdminUserListSerializer(
            User.objects.order_by("-created_at")[:8],
            many=True,
        ).data
        ...
        return Response(
            {
                ...
                "user_growth_over_time": build_user_growth_over_time(),
                "account_status_breakdown": [
                    {"label": "Active", "count": active_users},
                    {"label": "Inactive", "count": total_users - active_users},
                ],
                "email_verification_breakdown": [
                    {"label": "Verified", "count": verified_users},
                    {"label": "Unverified", "count": total_users - verified_users},
                ],
        ...
        with transaction.atomic():
            ...
            if "is_active" in validated:
                ...
                if (
                    ...
                    and User.objects.filter(is_superuser=True, is_active=True).count()
                    <= 1
                ):
                    log_audit_event(
                        "admin_user_update",
...
class AdminSettingsView(AdminAPIView):
    ...
    def patch(self, request):
        ...
        for field_name in BRANDING_ASSET_FIELDS:
            should_clear = bool(
                serializer.validated_data.get(f"clear_{field_name}", False)
            )
            if should_clear:
