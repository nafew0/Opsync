...
class SocialProvidersView(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        return Response(
            SocialProvidersResponseSerializer.build(), status=status.HTTP_200_OK
        )
...
class SocialAuthCallbackView(generics.GenericAPIView):
    ...
    def get(self, request, provider, *args, **kwargs):
        ...
        try:
            ...
            logger.warning(
                "Social login provider returned an OAuth error.", exc_info=True
            )
        ...
        if refresh_token:
            try:
                ...
                user = (
                    ...
                    .only("id", "email", "email_verified")
                    .first()
                )
                if (
                    user
                    and is_email_verification_required()
...
def logout_view(request):
    ...
    ).strip() or get_refresh_token_from_request(request)
    ...
    if refresh_token:
        try:
            ...
            logger.warning(
                "Refresh token blacklist failed during logout.", exc_info=True
            )
...
def delete_account_view(request):
    ...
    user = request.user
    delete_user_account(
        user, request=request, actor_user=user, audit_event="account_delete"
    )
