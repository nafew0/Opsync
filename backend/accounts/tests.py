...
def make_uploaded_image(
    name, *, size=(640, 360), image_format="PNG", color=(91, 45, 98)
):
    buffer = BytesIO()
...
class SignupProtectionTests(AccountsBaseTestCase):
    ...
    def test_signup_challenge_returns_registration_token_and_no_cache_when_captcha_disabled(
        self,
    ):
    ...
    def test_signup_challenge_survives_site_settings_schema_drift(
        self, mocked_get_or_create
    ):
        response = self.client.get("/api/auth/register/captcha/")
...
class RegistrationThrottleTests(AccountsBaseTestCase):
    ...
    def post_registration(
        self, index, *, client=None, remote_addr="127.0.0.1", **extra_headers
    ):
        payload = self.build_registration_payload(
            client=client,
...
class TokenRefreshSecurityTests(AccountsBaseTestCase):
    ...
    def test_cookie_refresh_rejects_untrusted_origin(self):
        ...
        self.assertEqual(
            response.data["detail"], "Refresh request origin is not allowed."
        )
...
class SocialProviderEndpointTests(AccountsBaseTestCase):
    ...
    def test_social_start_returns_authorization_url_for_enabled_provider(
        self, mocked_client
    ):
        self.site_settings.social_login_google_enabled = True
        self.site_settings.save(update_fields=["social_login_google_enabled"])
...
class SocialAuthCallbackViewTests(AccountsBaseTestCase):
    ...
    def test_social_callback_verification_redirect_clears_refresh_cookie(
        ...
        response = self.client.get(
            "/api/auth/social/facebook/callback/?state=test-state"
        )
...
class AdminUserDeletionTests(AccountsBaseTestCase):
    ...
    def create_user_with_related_data(
        self, *, username="target", email="target@example.com"
    ):
        user = User.objects.create_user(
            username=username,
            email=email,
