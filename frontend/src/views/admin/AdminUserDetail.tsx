...
export default function AdminUserDetail() {
  ...
  const handleStatusToggle = async () => {
    ...
    try {
      ...
      await refresh()
      toast({ title: 'User status updated', description: 'BdREN OpsSync saved the account status change.', variant: 'success' })
    } catch (err: unknown) {
      const e = err as AxiosError
      toast({ title: 'Status update failed', description: e.response?.data?.detail || 'BdREN OpsSync could not update this user right now.', variant: 'error' })
    } finally {
  ...
  const handleSendReset = async () => {
    ...
    try {
      ...
      toast({ title: 'Password reset sent', description: 'BdREN OpsSync emailed a secure password reset link to the user.', variant: 'success' })
    } catch (err: unknown) {
      const e = err as AxiosError
      toast({ title: 'Reset email failed', description: e.response?.data?.detail || 'BdREN OpsSync could not send the reset email.', variant: 'error' })
    } finally {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ['admin-users'] })
      await queryClient.invalidateQueries({ queryKey: ['admin-dashboard'] })
      toast({ title: 'User deleted', description: response.message || 'BdREN OpsSync permanently deleted this user account.', variant: 'success' })
      router.push('/admin/users')
    } catch (err: unknown) {
      const e = err as AxiosError
      toast({ title: 'Delete failed', description: e.response?.data?.detail || 'BdREN OpsSync could not delete this user right now.', variant: 'error' })
    } finally {
  ...
  if (error) {
    return <div className="theme-panel rounded-[1.8rem] p-6 text-sm text-rose-600">BdREN OpsSync could not load this user right now.</div>
  }
      ...
      <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        ...
        <Card className="theme-panel rounded-[1.8rem] border-0">
          <CardHeader>
            ...
            <CardDescription>Administrative recovery and access actions.</CardDescription>
          </CardHeader>
