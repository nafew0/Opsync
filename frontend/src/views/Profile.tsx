...
const SIGNATURE_MAX_SIZE_BYTES = 500 * 1024
...
export default function Profile() {
  ...
  useEffect(() => {
    // Sync authenticated user data into the editable profile form after account refreshes.
  ...
  return (
    <div className="ops-stack">
      <OpsPageHeader
        ...
        subtitle="Update the contact, identity, and signature details that appear across BdREN OpsSync requests and printable documents."
      />
          ...
          <CardContent className="space-y-6">
            <div className="flex flex-col items-center text-center">
              ...
              <button
                ...
                className="group relative inline-flex rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--ops-primary-200)] focus-visible:ring-offset-2"
                aria-label="Upload avatar"
              >
                <Avatar className="h-32 w-32 rounded-full border border-[color:var(--ops-ink-200)] shadow-[var(--ops-shadow-md)]">
                  <AvatarImage src={avatarPreview} alt={`${displayName} avatar`} className="object-cover" />
                  <AvatarFallback className="rounded-full bg-[color:var(--ops-primary-100)] text-3xl font-semibold text-[color:var(--ops-primary)]">
                    {getInitials(user, formData)}
                  </AvatarFallback>
                </Avatar>
                <span className="absolute bottom-1 right-1 inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/80 bg-[color:var(--ops-primary)] text-white shadow-lg transition group-hover:scale-105">
              ...
              <p className="mt-1 text-sm text-[color:var(--ops-ink-500)]">@{user?.username}</p>
            </div>

            <div className="rounded-[14px] border border-[color:var(--ops-ink-200)] bg-[color:var(--ops-ink-50)] px-4 py-3">
              <div className="flex items-center gap-3">
                <span className="ops-auth-note-mark">
                  <CalendarDays className="h-4 w-4" />
                </span>
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[color:var(--ops-ink-500)]">Member since</p>
                  <p className="text-sm font-medium text-[color:var(--ops-ink-900)]">{formatMemberSince(user?.created_at)}</p>
                </div>
              </div>
            </div>

            <p className="rounded-[14px] border border-dashed border-[color:var(--ops-ink-200)] bg-[color:var(--ops-paper)] px-4 py-3 text-sm text-[color:var(--ops-ink-500)]">
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  return (
                    <div key={field.id} className="space-y-2">
                      ...
                      <div className="relative">
                        <Icon className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[color:var(--ops-ink-400)]" />
                        <Input
                          id={field.id}
                          name={field.id}
                      ...
                      {field.description ? <p className="text-xs text-[color:var(--ops-ink-500)]">{field.description}</p> : null}
                    </div>
                  )
                })}
              </div>
              ...
              <div className="flex flex-col gap-3 border-t border-[color:var(--ops-ink-100)] pt-6 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-sm text-[color:var(--ops-ink-500)]">Profile changes are saved to your secure account immediately.</p>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                className="bg-[color:var(--ops-ink-50)]"
              />
            </div>
            <div className="space-y-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            />
            {user?.signature ? (
              <div className="rounded-[14px] border border-dashed border-[color:var(--ops-ink-200)] bg-white p-4 text-center">
                <img
                  src={resolveApiAssetUrl(user.signature)}
                  alt="Your signature"
                  className="mx-auto max-h-20 object-contain"
                />
                <p className="mt-2 text-xs text-[color:var(--ops-ink-500)]">This is how your signature will appear on documents.</p>
              </div>
            ) : (
              <div className="rounded-[14px] border border-dashed border-[color:var(--ops-ink-200)] bg-[color:var(--ops-ink-50)] p-8 text-center text-sm text-[color:var(--ops-ink-500)]">
            ...
            <div className="flex gap-3">
              ...
              {user?.signature ? (
                <Button
                  type="button"
                  variant="ghost"
                  className="rounded-full text-[color:var(--ops-danger)]"
